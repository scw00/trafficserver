...
class CacheWriteAgain : public CacheTestHandler
{
public:
  CacheWriteAgain(size_t size, const char *url) : CacheTestHandler()
  {
    auto rt = new CacheReadTest(size, this, url);
    ...
    build_hdrs(rt->info, url, "application/x-json");
    build_hdrs(wt->info, url, "application/x-json");
    ...
    SET_HANDLER(&CacheWriteAgain::start_test);
  }
  ...
  validate_content_type(CacheTestBase *base)
  {
    ...
    int len;
    const char *value = field->value_get(&len);
    REQUIRE(memcmp(value, "application/x-json", len) == 0);
  }
};

class test_Alternate_S_to_S_remove_S_write_S : public CacheTestHandler
{
public:
  test_Alternate_S_to_S_remove_S_write_S(size_t size, const char *url) : CacheTestHandler()
  {
    auto rt = new CacheReadTest(size, this, url);
    auto wt = new CacheWriteTest(size, this, url);
    ...
    SET_HANDLER(&test_Alternate_S_to_S_remove_S_write_S::start_test);
  }
...
public:
  ...
  cache_init_success_callback(int event, void *e) override
  {
    ...
    // 2. write an alternate and remove the first one
    test_Alternate_S_to_S_remove_S_write_S *ls = new test_Alternate_S_to_S_remove_S_write_S(SMALL_FILE, "http://www.scw11.com");
    // 3. read the first alternate (should open read failed)
    CacheAltReadAgain *read = new CacheAltReadAgain(SMALL_FILE, "http://www.scw11.com");
    // 4. write a new alternate and read this.
    CacheWriteAgain *write = new CacheWriteAgain(SMALL_FILE, "http://www.scw11.com");
